...
__device__ size_t atomicAggInc(atomic_ctr_t *ctr) {
  ...
  size_t warp_res;
  if(g.thread_rank() == 0)
    warp_res = atomicAdd(ctr, static_cast<atomic_ctr_t>(g.size()));
...
)
{
  ...
  if (threadIdx.x < j0 && blockIdx.x == 0) {
    if (row[threadIdx.x]) {
      out_col_ind[row_base + atomicAggInc(row_count)] = threadIdx.x;
    }
  }
  ...
  for(; j + 15 < num_cols; j += 16 * (blockDim.x * gridDim.x)){
    ...
    for (int k = 0; k < 16; ++k) {
      if (chunk.val.data[k]) {
        out_col_ind[row_base + atomicAggInc(row_count)] = j + k;
      }
    }
  }
  ...
  if (threadIdx.x < j1 && blockIdx.x == 0) {
    ...
    if (row[j]) {
      out_col_ind[row_base + atomicAggInc(row_count)] = j;
    }
  }
}
              ...
              cudaStream_t stream)
{
  ...
  thrust::uninitialized_fill(handle.get_thrust_policy(), row_counters, row_counters + batch_size, 0);
  ...
  cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocks_per_sm, adj_to_csr<Index_>, threads_per_block, 0);
